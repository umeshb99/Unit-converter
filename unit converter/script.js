...
function temp() {
  ...
  var totext =
    document.getElementById("ttemp").options[
      document.getElementById("ttemp").selectedIndex
    ].text;
  if (
    document.getElementById("tip").value == null ||
    document.getElementById("tip").value.length == 0
  )
    document.getElementById("top").value = "Please enter No. of Units";
  else if (from == 1 && to == 1)
    document.getElementById("top").value = ip + " " + totext;
  if (from == 1 && to == 2)
    document.getElementById("top").value =
      ((ip * 9) / 5 + 32 * 1).toFixed(2) + " " + totext;
  if (from == 1 && to == 3)
    document.getElementById("top").value =
      (ip * 1 + 273.15 * 1).toFixed(2) + " " + totext;
  if (from == 2 && to == 1)
    document.getElementById("top").value =
      (((ip - 32) * 5) / 9).toFixed(2) + " " + totext;
  if (from == 2 && to == 2)
    document.getElementById("top").value = ip + " " + totext;
  if (from == 2 && to == 3)
    document.getElementById("top").value =
      (((ip - 32) * 5) / 9 + 273.15).toFixed(2) + " " + totext;
  if (from == 3 && to == 1)
    document.getElementById("top").value =
      (ip - 273.15).toFixed(2) + " " + totext;
  if (from == 3 && to == 2)
    document.getElementById("top").value =
      (((ip - 273.15) * 9) / 5 + 32 * 1).toFixed(2) + " " + totext;
  if (from == 3 && to == 3)
    document.getElementById("top").value = ip + " " + totext;
}
function selunit() {
